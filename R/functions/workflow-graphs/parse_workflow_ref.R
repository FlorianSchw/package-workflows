...
# `lines` is the same split in two for diagram boxes. `key` identifies the
# exact call (with ref), `id` the workflow regardless of ref, as used in
# config/workflow-outcomes.yml. Returns NULL for anything else (a job
# without `uses:`).
parse_workflow_ref <- function(uses) {
  ...
  if (startsWith(uses, "./")) {
    ...
    return(list(local = TRUE, owner = NA, repo = NA, path = path, ref = NA,
                file = basename(path), key = path, id = path,
                label = basename(path), lines = basename(path)))
  }
  ...
  list(local = FALSE, owner = m[2], repo = m[3], path = m[4], ref = m[5],
       ...
       lines = c(sprintf("%s/%s", m[2], m[3]), sprintf("%s@%s", basename(m[4]), m[5])))
}
