...
- `.github/workflows/` — reusable workflows:
  ...
  - `package-release.yml` — semantic-release. Uses a caller-root
    ...
    fetched at `config-ref` (default: tag `v1`).
  ...
  - `apply-ruleset.yml` — not reusable: applies `rulesets/*.json` branch
    protection to the repos in `rulesets/repo-targets.json` (admin PAT).
- `.github/actions/` — composite actions used by the suggestion workflows:
  `anthropic-token`, `resolve-push-token`, `commit-updated-files`.
...
- **Auth: WIF, no static API key.** The `anthropic-token` action exchanges
  the job's GitHub OIDC token for a short-lived Anthropic token. Each caller
  repo needs a federation rule in the Claude Console (`subject_prefix:
  repo:<owner>/<repo>:*`) with a `claims` matcher on `job_workflow_ref` —
  i.e. per reusable workflow file, so a rule for `roxygen-suggest.yml` does
  not automatically cover `test-coverage-suggest.yml`. Only
  `anthropic-federation-rule-id` is a per-caller input; the org and
  service-account IDs live in the action.
...
- **Committing:** scripts write `updated_files.txt`; `commit-updated-files`
  commits exactly those files — per-file commits to the PR branch in
  `changed` mode, one sweep branch + PR against `dev` in `all` mode.
  `resolve-push-token` provides an App token when configured, so the
  commits trigger the caller's required checks.
...
- `all` mode of both suggestion workflows and all of
  `test-coverage-suggest.yml` have never run in real CI.
